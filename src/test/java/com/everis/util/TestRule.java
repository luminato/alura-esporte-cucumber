...
public class TestRule extends TestWatcher {
	...
	public static boolean userCadastrado;
				...
				if (errorMessage != null) {
					if (errorMessage.contains("FindFailed")) {
						try {
							...
							FileUtils.copyFile(new File(ImagePath.find(findFailedImage).getPath()),
									new File("target/report/html/img/" + findFailedImage));
							extentTest.info("FindFailed: " + findFailedImage,
									MediaEntityBuilder.createScreenCaptureFromPath("img/" + findFailedImage).build());
						} catch (Exception e) {
						}
					}
				}
